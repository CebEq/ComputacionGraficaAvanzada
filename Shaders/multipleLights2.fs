...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	...
	vec3 tex0Value = vec3(texture(tex0, our_uv));
	vec3 tex1Value = vec3(texture(tex1, our_uv));
	vec3 texMixValue = mix(tex0Value, tex1Value, 0.7);

	// Ambient
    vec3 ambient  = light.ambient * texMixValue;
  	
    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = light.diffuse * (diff * texMixValue);
    ...
    vec3 specular = light.specular * (spec * texMixValue);  
...
void main()
{
	vec4 colorText = texture(tex0, our_uv);
	if(colorText.a < 0.1)
		discard;
